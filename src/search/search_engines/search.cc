...
template bool SearchBase::check_goal<SparsePackedState>(
        ...
        const DBState &state, const SearchNode &node, const SearchSpace<SparsePackedState> &space, double thesis_time, double thesis_init, DynamicState the, double cleanup) const;

template bool SearchBase::check_goal<ExtensionalPackedState>(
        const Task &task, const SuccessorGenerator &generator, clock_t timer_start,
        const DBState &state, const SearchNode &node, const SearchSpace<ExtensionalPackedState> &space, double thesis_time, double thesis_init, DynamicState the, double cleanup) const;
